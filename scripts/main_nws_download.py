...
def create_nws_gif(nws_ds, cmap, cbar_label, data_title):
    ...
    storage = create_client(f"{database_url}/storage/v1", api_key, is_async=Literal[True])
...
def generate_tiles_from_zarr(ds, layer_name, supabase_prefix):
    ...
    storage = create_client("https://rndqicxdlisfpxfeoeer.supabase.co/storage/v1", api_key, is_async=Literal[True])
    bucket_name = "maps"
